"""
train_model_v8.py
父×母父の組み合わせ（配合ニック）特徴量を追加したv7モデルAの実験版

v7モデルAからの変更点:
  - nick = sire + "×" + bms_name の組み合わせごとに
    smooth_mean / smooth_over200 / count を追加
    - 他クラブデータには sire 列がないため、シルクの学習データ
      （601頭・ユニーク組み合わせ162種）のみで集計
    - サンプルが極めて薄い（中央値1頭）ため、過学習が懸念される

出力:
  models/lgbm_model_A_v8.pkl
  models/feature_importance_A_v8.csv
  data/valid_predictions_v8.csv
"""
...
def build_aggs(pool, bms_pool, nick_pool):
    ...
    # nick（父×母父）はシルクの学習データのみで集計（他クラブにsire列なし）
...
def save_importance(model, feature_cols):
    ...
    fi.to_csv(MODEL_DIR / "feature_importance_A_v8.csv", index=False, encoding="utf-8-sig")
    print("\n--- 特徴量重要度 TOP10 ---")
...
def main():
    ...
    nick_pool = silk_train[["nick", "kaishuu_rate"]]
    ...
    print(f"=== nick集計プール: {len(nick_pool)} 頭 (nick有: {nick_pool['nick'].notna().sum()}) ===")
    ...
    }, MODEL_DIR / "lgbm_model_A_v8.pkl")
    print("\nモデル保存: models/lgbm_model_A_v8.pkl")

    df_eval.to_csv(DATA_DIR / "valid_predictions_v8.csv", index=False, encoding="utf-8-sig")
    print("予測結果保存: data/valid_predictions_v8.csv")

    print("\n" + "="*60)
    print("=== v7 vs v8（父×母父ニック追加）比較 ===")
    print("="*60)
    print(f"{'指標':<28} {'v8':>10}")
    print("-"*60)
