"""
train_model_v14.py
v9に「母（dam）」単体と「牧場×調教師」組み合わせの特徴量を追加した版

背景:
  merged_train.csv には fetch_mother_name.py で取得済みの mother_name
  （母馬名）列が既に存在するが、v9まで一度もモデルの特徴量として
  使われていなかった。bms_name（母父）は使っているが、母そのものの
  実績（同じ母から出た他の子の回収率）は別の情報を持ちうる。
  他クラブ分の母馬名は fetch_mother_other.py で今回のセッション中に
  2,259頭ぶん取得済み（元々はキャロット測尺マッチング用だったが
  集計プール拡張にも転用できる）。

v9からの変更点:
  - dam（母馬名）を sire/trainer/farm/bms_name と同じ要領で
    smooth_mean特徴量化（集計プールはシルク+他クラブ）
  - farm_trainer（牧場×調教師の組み合わせ）を nick と同じ要領で追加
    （生産・育成のコンビネーション効果を狙う）

出力:
  models/lgbm_model_A_v14.pkl
  models/feature_importance_A_v14.csv
  data/valid_predictions_v14.csv
"""
...
def load_other():
    ...
    if mother_path.exists():
        ...
        df = pd.merge(df, mother, on="horse_id", how="left")
    else:
        df["mother_name"] = np.nan
    return df
...
def build_aggs(pool, bms_pool, nick_pool, mother_pool, farm_trainer_pool):
    ...
    # mother_name（母そのもの、v14で追加）
    mother_global_mean = mother_pool["kaishuu_rate"].mean()
    result = smooth_mean(mother_pool, "mother_name", "kaishuu_rate", mother_global_mean)
    if len(result) > 0:
        n_used = mother_pool["mother_name"].notna().sum()
        aggs["mother_name"] = result
        print(f"  mother_name: {len(result)} ユニーク / {n_used} 頭")

    # farm_trainer（牧場×調教師、v14で追加）
    ft_global_mean = farm_trainer_pool["kaishuu_rate"].mean()
    result = smooth_mean(farm_trainer_pool, "farm_trainer", "kaishuu_rate", ft_global_mean)
    if len(result) > 0:
        n_used = farm_trainer_pool["farm_trainer"].notna().sum()
        aggs["farm_trainer"] = result
        print(f"  farm_trainer: {len(result)} ユニーク / {n_used} 頭")
...
def save_importance(model, feature_cols):
    ...
    fi.to_csv(MODEL_DIR / "feature_importance_A_v14.csv", index=False, encoding="utf-8-sig")
    print("\n--- 特徴量重要度 TOP10 ---")
...
def main():
    ...
    silk_train = add_farm_trainer(silk_train)
    silk_valid = add_farm_trainer(silk_valid)
    other_train = add_farm_trainer(other_train)

    print(f"シルク 学習:{len(silk_train)} / 検証:{len(silk_valid)}")
    print(f"他クラブ(学習年度のみ): {len(other_train)} 頭 (sire有: {other_train['sire'].notna().sum()}, mother_name有: {other_train['mother_name'].notna().sum()})")
    ...
    mother_pool = pd.concat([
        silk_train[["mother_name", "kaishuu_rate"]],
        other_train[["mother_name", "kaishuu_rate"]],
    ], ignore_index=True)
    farm_trainer_pool = pd.concat([
        silk_train[["farm_trainer", "kaishuu_rate"]],
        other_train[["farm_trainer", "kaishuu_rate"]],
    ], ignore_index=True)
    ...
    print(f"=== mother集計プール: {len(mother_pool)} 頭 (mother_name有: {mother_pool['mother_name'].notna().sum()}) ===")
    print(f"=== farm_trainer集計プール: {len(farm_trainer_pool)} 頭 (有: {farm_trainer_pool['farm_trainer'].notna().sum()}, ユニーク: {farm_trainer_pool['farm_trainer'].nunique()}) ===")
    aggs = build_aggs(pool, bms_pool, nick_pool, mother_pool, farm_trainer_pool)
    ...
    print("  ※ 単一seed・生の回収率だけでの改善判定はノイズに引きずられるため、")
    print("    以降の実験はこの数値を『正式なベースライン』として比較すること")
    stable = evaluate_stable(
        lambda seed: train_lgbm(X_tr, y_tr, cw, random_state_override=seed),
        X_tr, y_tr, X_vl, vl, label="v14",
    )
    ...
    }, MODEL_DIR / "lgbm_model_A_v14.pkl")
    print("\nモデル保存: models/lgbm_model_A_v14.pkl")

    df_eval.to_csv(DATA_DIR / "valid_predictions_v14.csv", index=False, encoding="utf-8-sig")
    print("予測結果保存: data/valid_predictions_v14.csv")

    print("\n" + "="*60)
    print("=== v9 vs v14(mother_name + farm_trainer追加) 比較 ===")
    print("="*60)
    print(f"{'指標':<28} {'v14':>10}")
    print("-"*60)
