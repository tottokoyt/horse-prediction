"""
train_model_v10.py
モデルAの実学習行に、測尺が取得できた他クラブ馬を追加したv9の改訂版

v9からの変更点:
  - 他クラブは集計プール（スムージング統計）にしか使われておらず、
    LightGBM本体の実学習行はシルク単独634頭のままだった
    （他クラブには測尺=体高/胸囲/管囲/体重の列が無いため）
  - merge_carrot_scale.py で Wayback Machine 上のキャロットクラブ
    公式アーカイブ（募集時測尺）を母馬名+募集年度でマッチングし、
    data/other_scale_cache.csv として242頭ぶん復元
  - 測尺が復元できた他クラブ馬を実学習行（X_tr/y_tr）にシルクと合わせて投入
    （検証セットは従来通りシルク単独156頭のまま、v9と比較可能に保つ）

出力:
  models/lgbm_model_A_v10.pkl
  models/feature_importance_A_v10.csv
  data/valid_predictions_v10.csv
"""
...
def save_importance(model, feature_cols):
    ...
    fi.to_csv(MODEL_DIR / "feature_importance_A_v10.csv", index=False, encoding="utf-8-sig")
    print("\n--- 特徴量重要度 TOP10 ---")
...
def main():
    ...
    }, MODEL_DIR / "lgbm_model_A_v10.pkl")
    print("\nモデル保存: models/lgbm_model_A_v10.pkl")

    df_eval.to_csv(DATA_DIR / "valid_predictions_v10.csv", index=False, encoding="utf-8-sig")
    print("予測結果保存: data/valid_predictions_v10.csv")

    print("\n" + "="*60)
    print("=== v9(実学習シルクのみ634頭) vs v10(実学習+他クラブ測尺復元分) 比較 ===")
    print("="*60)
    print(f"{'指標':<28} {'v10':>10}")
    print("-"*60)
