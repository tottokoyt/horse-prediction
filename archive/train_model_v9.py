"""
train_model_v9.py
父×母父の組み合わせ（配合ニック）特徴量を、他クラブを含めた大きなプールで
再検証したv8の改訂版

v8からの変更点:
  - fetch_sire_other.py で取得した他クラブの sire（sire_other_cache.csv）を
    jisseki_other_all.csv にマージし、nick_pool をシルク単独（601頭）から
    シルク＋他クラブ学習年度分に拡張
  - サンプル数が増えることで nick の smooth_mean 過学習が緩和されるか検証

出力:
  models/lgbm_model_A_v9.pkl
  models/feature_importance_A_v9.csv
  data/valid_predictions_v9.csv
"""
...
def save_importance(model, feature_cols):
    ...
    fi.to_csv(MODEL_DIR / "feature_importance_A_v9.csv", index=False, encoding="utf-8-sig")
    print("\n--- 特徴量重要度 TOP10 ---")
...
def main():
    ...
    }, MODEL_DIR / "lgbm_model_A_v9.pkl")
    print("\nモデル保存: models/lgbm_model_A_v9.pkl")

    df_eval.to_csv(DATA_DIR / "valid_predictions_v9.csv", index=False, encoding="utf-8-sig")
    print("予測結果保存: data/valid_predictions_v9.csv")

    print("\n" + "="*60)
    print("=== v8(nickシルクのみ) vs v9(nick他クラブ込み) 比較 ===")
    print("="*60)
    print(f"{'指標':<28} {'v9':>10}")
    print("-"*60)
