"""
train_model_v12.py
v11（club_name分布差補正）に他クラブ行のダウンウェイトを加えた版

v11で判明したこと:
  club_name補正で上位25%平均回収率は73.9%→77.1%まで持ち直したが、
  v9（シルク単独634頭）の87.0%にはまだ届かない。実際に silk と
  carrot(matched 242頭)の測尺分布を比較すると差は小さく（体重のみ+14kg程度）、
  「測尺分布の差」より「クラブごとの特徴量〜回収率の関係そのものの違い」が
  尾を引いている可能性が高い。

v11からの変更点:
  - 他クラブ由来の学習行（227頭）の sample_weight を silk 行より
    軽くする（OTHER_CLUB_WEIGHT）。木の分岐がシルク行を優先しつつ、
    他クラブ行からは補助的にしか学習しないようにする

出力:
  models/lgbm_model_A_v12.pkl
  models/feature_importance_A_v12.csv
  data/valid_predictions_v12.csv
"""
...
def train_lgbm(X_train, y_train, cw, source_weight=None):
...
def cross_validate_model(X, y, cw, source_weight=None, n_splits=5):
    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
    aucs = []
    for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y)):
        X_tr, X_val = X.iloc[tr_idx], X.iloc[val_idx]
        y_tr, y_val = y.iloc[tr_idx], y.iloc[val_idx]
        sw_tr = source_weight[tr_idx] if source_weight is not None else None
        m  = train_lgbm(X_tr, y_tr, cw, sw_tr)
...
def save_importance(model, feature_cols):
    ...
    fi.to_csv(MODEL_DIR / "feature_importance_A_v12.csv", index=False, encoding="utf-8-sig")
    print("\n--- 特徴量重要度 TOP10 ---")
...
def main():
    ...
    source_weight = np.where(tr["club_name"] == "silk", 1.0, OTHER_CLUB_WEIGHT)
    print(f"\n他クラブ行のsample_weight倍率: {OTHER_CLUB_WEIGHT}")

    cw = get_class_weight(y_tr)
    print(f"クラス重み: { {k: round(v,2) for k,v in sorted(cw.items())} }")

    print("\n=== クロスバリデーション (5fold) ===")
    cv_auc = cross_validate_model(X_tr, y_tr, cw, source_weight)

    print("\n=== 全データで学習 ===")
    model = train_lgbm(X_tr, y_tr, cw, source_weight)
    ...
    }, MODEL_DIR / "lgbm_model_A_v12.pkl")
    print("\nモデル保存: models/lgbm_model_A_v12.pkl")

    df_eval.to_csv(DATA_DIR / "valid_predictions_v12.csv", index=False, encoding="utf-8-sig")
    print("予測結果保存: data/valid_predictions_v12.csv")

    print("\n" + "="*60)
    print("=== v11(重み均等) vs v12(他クラブ行ダウンウェイト) 比較 ===")
    print("="*60)
    print(f"{'指標':<28} {'v12':>10}")
    print("-"*60)
